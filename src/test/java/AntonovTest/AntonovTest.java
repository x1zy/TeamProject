...
public class AntonovTest {
    ...
    public void testInitialValues() {
        Bear bear = new Bear("Antonov", 0);
        assertEquals("Herald", bear.rank, "Начальный ранг должен быть Herald");
        assertEquals(1, bear.pts, "Начальный уровень PTS должен быть 1");
    ...
    @Test
    public void testWinPts() {
        Bear bear = new Bear("Antonov", 0);
        bear.winPts();
        assertTrue(bear.pts >= 51 && bear.pts <= 101, "PTS после победы должен быть в диапазоне 51–101");
        assertEquals(1, bear.numberOfGames, "Количество игр должно увеличиться на 1");
    }
